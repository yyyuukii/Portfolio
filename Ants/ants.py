...
class Ant(Insect):
    ...
    def add_to(self, place: Place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            existing = place.ant
            if existing.is_container and existing.can_contain(self):
                existing.store_ant(self)
            elif self.is_container and self.can_contain(existing):
    ...
    def double(self):
        ...
        if not hasattr(self, 'damage'):
            return
        if getattr(self, '_doubled', False):
            return
        self.damage *= 2
        self._doubled = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        while current is not None and not current.is_hive:
            if self.lower_bound <= distance <= self.upper_bound and current.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        if self.health <= 0 and current_place is not None:
            for bee in current_place.bees[:]:
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant: Ant):
        # BEGIN Problem 8a
        assert self.can_contain(ant), "Cannot contain {}".format(ant)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        """
        # BEGIN Problem 12
        if not hasattr(self, '_is_true_queen'):
            if not QueenAnt._true_queen_created:
                self._is_true_queen = True
                QueenAnt._true_queen_created = True
            else:
                self._is_true_queen = False

        if not self._is_true_queen:
        ...
        p = self.place.exit
        while p is not None and not getattr(p, 'is_hive', False):
            ant = p.ant
            if ant is not None:
                if getattr(ant, 'is_container', False):
                    ant.double()
                    if getattr(ant, 'ant_contained', None):
    ...
    def reduce_health(self, amount: float):
        ...
        super().reduce_health(amount)
        if self.health <= 0 and getattr(self, '_is_true_queen', False):
            ants_lose()
        # END Problem 12
